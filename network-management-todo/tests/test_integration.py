    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('success'):
                ...
                print(f"✅ 创建任务成功: {todo_id}")
                return todo_id
            else:
                print(f"❌ 创建任务失败: {data.get('message')}")
        else:
            print(f"❌ 创建任务请求失败: {response.status_code}")
    except Exception as e:
        print(f"❌ 创建任务异常: {e}")
    return None
...
def test_get_todos(base_url: str, token: str) -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('success'):
                todos = data.get('data', [])
                print(f"✅ 获取任务列表成功: {len(todos)} 个任务")
                return True
            else:
...
def test_update_todo(base_url: str, token: str, todo_id: str) -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ 更新任务请求失败: {response.status_code}")
    except Exception as e:
        print(f"❌ 更新任务异常: {e}")
    return False
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('success'):
                ...
                print(f"❌ 删除任务失败: {data.get('message')}")
        else:
            print(f"❌ 删除任务请求失败: {response.status_code}")
    except Exception as e:
        print(f"❌ 删除任务异常: {e}")
    return False
