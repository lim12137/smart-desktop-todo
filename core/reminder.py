...
class ReminderService:
    ...
    def _get_pending_reminders(self) -> List[Tuple[TodoItem, str, str]]:
        ...
        urgent_days = self.data_manager.get_config("urgent_days", 3)
        warning_days = self.data_manager.get_config("warning_days", 7)

        # 定义检查规则：(阈值范围, remind_type, 是否启用配置键)
        rules = [
            (lambda d: d < 0, "overdue", remind_overdue_enabled, "reminded_overdue"),
            (lambda d: 0 <= d <= urgent_days, "3day", remind_3day_enabled, "reminded_3day"),
            (lambda d: urgent_days < d <= warning_days, "7day", remind_7day_enabled, "reminded_7day"),
        ]
